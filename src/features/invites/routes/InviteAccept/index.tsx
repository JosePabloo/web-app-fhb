...
// NOTES: Reuses the cached public invite lookup from the landing step when available.
...
import { usePublicInviteQuery } from '../../hooks/usePublicInviteQuery';
...
export default function InviteAccept() {
  ...
  const { data: invite, isPending: loadingInvite, isError } = usePublicInviteQuery(
    inviteId || undefined,
  );
  ...
  const error = !inviteId ? 'Missing invite link.' : isError ? 'This invite is invalid or expired.' : '';
  ...
  const tenantLabel = useMemo(
    () => invite?.tenantName ?? invite?.tenantId ?? 'your team',
    [invite],
  );
  ...
  const handleRegister = useCallback(async () => {
    ...
    try {
      ...
      console.log(
        'Registering credential for inviteId:',
        username,
        email,
        phoneNumber ? `with phone: ${phoneNumber}` : 'without phone',
      );
      await registerCredential(username, email, undefined, inviteId);
      navigate('/dashboard');
    } catch (err) {
  ...
  return (
    <StepGuard validateState={(state) => state?.phoneValidated === true} fallbackPath="/invite">
      <PageWrapper>
        <Box
          sx={{
            minHeight: '100vh',
                ...
                {loadingInvite ? (
                  ...
                  <>
                    <Typography variant="body2" color="text.secondary">
                      you&apos;re joining <strong>{tenantLabel}</strong>. Complete your details and
                      register a passkey to continue.
                    </Typography>
                    {invite?.maskedEmail ? (
                      <Typography variant="body2" color="text.secondary">
